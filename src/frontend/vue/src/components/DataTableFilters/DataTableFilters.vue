  ...
  <section class="ui-filter_section">
    ...
    <form class="ui-filter_wrap" v-show="filtersVisible === true">
      ...
      <div class="ui-filter-flex_wrap">
        ...
        <button @click="() => { filtersVisible = false; $emit('clickActionButton', filters) }">{{ options.actionButtonText }}</button>
      </div>
    </form>
  </section> 
...
<script setup>
  ...
  const filters = ref({
    ...
    sortBy: props.options?.sortByOptions?.find(sortByOption => sortByOption.defaultSortBy === true)?.key,
    sortOrder: "1"
  });
